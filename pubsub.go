...
type PubSub struct {
	...
	// Should auto relay topic when peer connected
	relayAll bool

	// Cancel auto relay when topic is not active, set to 0 to disable this feature
	relayTimeout time.Duration

	myTopicsLastActiveTime map[string]time.Time
...
func NewPubSub(ctx context.Context, h host.Host, rt PubSubRouter, opts ...Option) (*PubSub, error) {
	ps := &PubSub{
		...
		eval:                   make(chan func()),
		myTopicsLastActiveTime: make(map[string]time.Time),
...
func WithRelayAllWithTimeout(timeout time.Duration) Option {
	return func(ps *PubSub) error {
		ps.relayAll = true
		ps.relayTimeout = timeout
		return nil
	}
}
...
func (p *PubSub) processLoop(ctx context.Context) {
	...
	cancelTimeoutRelay := make(chan struct{}, 1)
	if p.relayAll && p.relayTimeout > 0 {
		go p.cancelTimeoutRelayLoop(ctx, cancelTimeoutRelay)
	}
	...
	for {
		...
		case <-cancelTimeoutRelay:
			p.cancelTimeoutRelay()
...
func (p *PubSub) cancelTimeoutRelayLoop(ctx context.Context, cancelTimeoutRelay chan struct{}) {
	log.Info("Run cancelTimeoutRelayLoop")
	ticker := time.NewTicker(time.Minute * 10)
	defer func() {
		ticker.Stop()
		log.Info("Exit cancelTimeoutRelayLoop")
	}()
	...
	for {
		...
		case <-ticker.C:
			select {
			case cancelTimeoutRelay <- struct{}{}:
			default:
			}
		case <-ctx.Done():
			return
...
func (p *PubSub) handleAddTopic(req *addTopicReq) {
	...
	p.myTopicsLastActiveTime[topicID] = time.Now()
	req.resp <- topic
}
...
func (p *PubSub) cancelTimeoutRelay() {
	now := time.Now()
	removed := 0
	for t, count := range p.myRelays {
		var cancel bool
		var topic *Topic
		var ok bool
		if topic, ok = p.myTopics[t]; ok {
			if time, ok := p.myTopicsLastActiveTime[t]; ok {
				if now.Sub(time) > p.relayTimeout {
					cancel = true
				}
			} else {
				cancel = true
		...
		if cancel {
			removed++
			log.Infof("Cancel relay timeout topic %s", t)
			for i := 0; i < count; i++ {
				p.handleRemoveRelay(t)
			}
			req := &rmTopicReq{topic, make(chan error, 1)}
			p.handleRemoveTopic(req)
			<-req.resp
		}
	}
	log.Infof("Cancel timeout relays finished, removed %d topics, current have %d", removed, len(p.myRelays))
}
...
func (p *PubSub) handleIncomingRPC(rpc *RPC) {
				...
				if topic, ok := p.myTopics[t]; ok {
					...
					if p.relayAll {
						go func(t string) {
							...
							if err != nil {
								log.Infof("Relay topic %s failed, error: %v", t, err)
							} else {
								log.Infof("Relay topic %s", t)
							}
						}(t)
					}
				}
	...
	case AcceptAll:
		for _, pmsg := range rpc.GetPublish() {
			...
			p.myTopicsLastActiveTime[pmsg.GetTopic()] = time.Now()
...
func (p *PubSub) Eval(f func()) error {
	select {
	case p.eval <- f:
		return nil
...
// ListPeers returns a list of peers we are connected to in the given topic.
func (p *PubSub) ListMeshPeers(topic string) []peer.ID {
	if rt, ok := p.rt.(*GossipSubRouter); !ok {
		return p.ListPeers(topic)
	} else {
		out := make(chan []peer.ID)
		p.Eval(func() {
			var peers []peer.ID
			for p := range rt.MeshPeers(topic) {
				peers = append(peers, p)
			}
			out <- peers
		})
